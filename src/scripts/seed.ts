 ...
 *
 * Populates Payload with the 11 initial tools.
...
const SEED_TOOLS: SeedTool[] = [
  ...
  },
  {
    ...
    name: {
      ...
      fr: 'Lecteur de Donnees Extranet',
  ...
  },
  {
    ...
    name: {
      ...
      fr: 'Generateur de Formulaire Elementor',
      ca: 'Generador de Formulari Elementor',
      pt: 'Gerador de Formulario Elementor',
    },
    description: {
...
async function seed() {
  ...
  if (totalDocs > 0) {
    console.log(`⚠️  Found ${totalDocs} existing tools. Skipping seed to avoid duplicates.`)
    console.log('   Delete tools from /admin first if you want to re-seed.')
    process.exit(0)
  }
  ...
  for (const tool of SEED_TOOLS) {
    ...
    const existing = await payload.find({
      collection: 'tools',
      where: { url: { equals: tool.url }, order: { equals: tool.order } },
      limit: 1,
    })
  ...
  console.log(`\n✨ Seed complete — ${SEED_TOOLS.length} tools created.`)
  process.exit(0)
}
