...
function getViewMode(pathname: string): ViewMode {
  if (/\/collections\/categories\/create/.test(pathname)) return 'create'
  if (/\/collections\/categories\/\d+/.test(pathname)) return 'edit'
  return null
}

export function MiraiCategoryCreateHeader() {
  ...
  useEffect(() => {
    ...
    const bodyClass = mode === 'create' ? 'mirai-categories-create' : 'mirai-categories-edit'
  ...
  const pageLabel = isEdit ? 'Edit category' : 'New category'
